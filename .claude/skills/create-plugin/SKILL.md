...
Always build plugin zips with `scripts/New-PluginZip.ps1` at the repo root
instead:

```powershell
.\scripts\New-PluginZip.ps1 -PluginDir .\plugins\<plugin-name> -OutFile .\<plugin-name>.zip
```
...
cheaper to fix before even building the zip.
...
`plugins/pmcro-trail/skills/initialize/scripts/New-Trail.ps1`, same as any
other cycle in this repo. Don't let a real, reusable finding live only in
chat history.
